...
void ClientControl::call_execv(const char *programname, vector<string> args_vector) {
    ...
    const char **argv = new const char* [args_vector.size()+2];   // extra room for program name and sentinel
    argv [0] = programname;         // by convention, argv[0] is program name
    for (unsigned int j = 0;  j < args_vector.size()+1;  ++j)     // copy args
            argv [j+1] = args_vector[j].c_str();

    argv [args_vector.size()+1] = NULL;  // end of arguments sentinel is NULL
