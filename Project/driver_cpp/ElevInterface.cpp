...
void ElevInterface::set_button_lamp(elev_button_type_t button, int floor, int value) {
    assert(floor >= 0);
...
void ElevInterface::set_floor_indicator(int floor) {
    assert(floor >= 0);
    assert(floor < N_FLOORS);
...
void ElevInterface::set_door_open_lamp(int value) {
    if (value) {
...
void ElevInterface::set_stop_lamp(int value) {
    if (value) {
...
int ElevInterface::get_button_signal(elev_button_type_t button, int floor) {
    assert(floor >= 0);
    assert(floor < N_FLOORS);
    assert(button >= 0);
    assert(button < N_BUTTONS);
...
int ElevInterface::get_stop_signal(void) {
    return io_read_bit(STOP);
}


int ElevInterface::get_obstruction_signal(void) {
